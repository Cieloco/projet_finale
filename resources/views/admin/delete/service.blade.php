<x-master title="La suppression de service {{ $service->name }}">
        ...
        <div class="card mb-4 border-0 shadow bg-white mt-5 rounded-top-2">
            ...
            <div class="card-body my-2">
                <div class="px-5 py-5 bg-white shadow-sm mx-auto">
                    <h5>Êtes-vous sûr de vouloir supprimer ce service ? Cette action est irréversible.</h5>                    <div class="d-flex justify-content-evenly mt-5">
                        <form action="{{ route('services.destroy', $service->id) }}" method="post">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn btn-danger px-4 py-2 me-5">Supprimer</button>
                            <a class="btn btn btn-info text-white px-4 py-2 ms-5" href="{{ route('services.index') }}">
                                Cancel
                            </a>
                        </form>
                    </div>
